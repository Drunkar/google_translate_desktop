...
# URLのベース部分 (tl は動的に決定)
BASE_TRANSLATE_URL_TEMPLATE = "https://translate.google.com/?hl={ui_lang}&sl={source_lang}&tl={target_lang}&op=translate&text={encoded_text}"
...
app_quit = False
# visible_window は Api クラス内で管理する
...
def listen_keyboard():
    ...
    try:
        ...
        hooked = True
        print("キーボードフックを設定しました。(Ctrl+C)")
        while not app_quit:
            time.sleep(0.5)
    except Exception as e:
         print(f"\n*** キーボードフックエラー: {e} ***")
         if platform.system() == "Linux": print("sudo試行 or inputグループ確認")
         elif platform.system() == "Windows": print("管理者権限試行")
         global hidden_window
         if hidden_window:
             try: hidden_window.evaluate_js('window.pywebview.api.shutdown_app()')
             except: os._exit(1)
         else: os._exit(1)
    finally:
        if hooked:
    ...
    if current_time - last_ctrl_c_time < DOUBLE_PRESS_TIMEOUT:
        ...
        try:
            ...
            if clipboard_content and clipboard_content.strip():
                ...
                else: print("エラー: 非表示ウィンドウ未初期化。")
            elif clipboard_content is None: print("クリップボードアクセス失敗(None)。")
            else: print("クリップボード空。")
        except Exception as e: print(f"クリップボード/JS評価エラー: {e}")
        last_ctrl_c_time = 0
    else:
...
class Api:
    ...
    def _handle_window_closed(self):
        ...
        if current_ref: print(f"表示用ウィンドウ closed. Clearing ref.")
        self.visible_window = None
    ...
    def create_or_focus_translate_window(self, text):
        ...
        if langdetect_available:
            ...
            except LangDetectException: print(f"言語検出失敗 -> デフォルトターゲット")
            except Exception as e_detect: print(f"言語検出エラー: {e_detect} -> デフォルトターゲット")
        # (言語検出ロジックここまで)
        ...
        try:
            ...
            final_url = BASE_TRANSLATE_URL_TEMPLATE.format(
                ui_lang=UI_LANG, source_lang=SOURCE_LANG,
                target_lang=effective_target_lang, encoded_text=encoded_text
            )
            print(f"目標URL (max 100): {final_url[:100]}...")
            if len(final_url) > 2000: print("警告: URL長 > 2000")
            ...
            if self.visible_window and platform.system() == "Windows" and win32gui:
                try:
                    ...
                    if window_to_reuse_hwnd != 0:
                        print(f"既存ウィンドウをタイトルで発見 (HWND: {window_to_reuse_hwnd})。内部参照も有効なため再利用します。")
            ...
            if can_reuse and self.visible_window:
                ...
                try:
                    ...
                    if platform.system() == "Windows" and win32gui and win32api and win32con and window_to_reuse_hwnd != 0:
                        try:
                            ...
                            try: win32api.keybd_event(win32con.VK_MENU, 0, win32con.KEYEVENTF_KEYUP, 0)
                            except: pass
                    if not activated:
    ...
    def shutdown_app(self):
        ...
        app_quit = True
        if hidden_window:
            try: hidden_window.destroy()
            except Exception as e: print(f"非表示ウィンドウ破棄エラー: {e}")
        print("メインスレッド終了を試みます...")
